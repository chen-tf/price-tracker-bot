...
def sync_price():
    ...
    import pt_bot
    for good_info in pt_repository.find_all_good():
        good_id = None
        try:
...
def disable_not_active_user_sub_good():
    active_user_ids = pt_repository.find_all_active_user()
    if not active_user_ids:
        return

    import pt_bot
    inactive_user_ids = tuple(str(result[1]) for result in active_user_ids if pt_bot.is_blocked_by_user(str(result[0])))

    pt_repository.disable_user_and_user_sub_goods(inactive_user_ids)
